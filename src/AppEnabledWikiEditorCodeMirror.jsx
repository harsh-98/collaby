...
export default class AppEnabledWikiEditorCodeMirror extends React.Component {
  ...
  async componentDidMount() {
    ...
    if (this.props.roomName) {
      // this.socket = io(`https://websocket.carboy90.hasura-app.io/`);
      this.socket = io(`http://localhost:1234`);
     await this.socket.emit('create', {
         userid: JSON.parse(document.cookie.split("hasura_auth_uikit=")[1].split("}}")[0]+"}}")['user_info']['hasura_id'],
         pagename: this.props.roomName
      });
